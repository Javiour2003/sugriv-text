...
class MaskedMultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        if mask is not None:
          mask = mask.unsqueeze(1)  # Add extra dimension for num_heads
          scores = scores.masked_fill(mask == 0, 0)
        else:
            print('mask is None!!!')  # Print a message if the mask is None
        ...
        weighted_sum = torch.matmul(attention_weights, V)  # (batch_size, num_heads, query_len, head_dim)

        # Concatenate heads and linear transformation # something wrong here ?
        weighted_sum = weighted_sum.transpose(1, 2).contiguous().view(batch_size, -1, self.d_model)  # (batch_size, query_len, d_model)
